...
- Связь с персистенцией (**QueryStore** — **файловый стор, per-user**; позже опц.
  SQLite за абстракцией стора) и нижним табом **Query Logs**.
- **Восстановление незавершённых запросов после рестарта** (принято — закладываем
  сразу): что персистится, как QueryManager переподключается к выполняющемуся
  запросу или корректно его завершает после перезапуска.
